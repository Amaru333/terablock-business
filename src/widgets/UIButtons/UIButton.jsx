...
function UIButton({ children, type, width, onClick, ml0 = false, style, onMouseOut, onMouseOver }) {
  if (type == "primary") {
    return (
      <button onMouseOver={onMouseOver} onMouseOut={onMouseOut} className={[`bg-primaryBlue text-white px-4 py-2 ${!ml0 && "mx-2"}`, UIButtonStyles.button, UIButtonStyles.button_primary].join(" ")} style={{ width: width }} onClick={onClick}>
        {children}
      </button>
    );
